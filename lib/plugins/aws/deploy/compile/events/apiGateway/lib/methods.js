    ...
    _.forEach(this.serverless.service.functions, (functionObject, functionName) => {
      functionObject.events.forEach(event => {
        if (event.http) {
          ...
          const resourceLogicalId = (path === "/") ? "" : this.resourceLogicalIds[path];
          const normalizedMethod = method[0].toUpperCase() +
            method.substr(1).toLowerCase();
          const extractedResourceId = (path === "/") ? "" : resourceLogicalId.match(/ResourceApigEvent(.*)/)[1];
          ...
          const outputEndpointTemplate = `
          {
            ...
            "Value": { "Fn::Join" : [ "", [ "${method.toUpperCase()} - https://", { "Ref": "RestApiApigEvent" },
              ".execute-api.${this.options.region}.amazonaws.com/${this.options.stage}${hackPath}"] ] }
          }`;
